typedef struct {
  ...
  unsigned short * value;       // array of real values for each gene
...
void showIndividual(INDIVIDUAL * ind);
void showRealValues(INDIVIDUAL * ind);
...
INDIVIDUAL * allocIndividual() {
  ...
  ind->value = (unsigned short *)malloc(NPARAMS*sizeof(unsigned short));
